...
def compute_state_visitation_frequencies(
    ...
    if isinstance(T, csr_matrix):
        ...
        # Policy: Shape (n_states,), Entries ∈ [0, n_actions)
        policy_flat = np.zeros(n_states * n_actions)
        for s in range(n_states):
            a = policy[s]
            policy_flat[s * n_actions + a] = 1.0
...
def run_ng_russell(
    ...
    log_memory("Before Projection IRL")
    ...
    log_memory("After Projection IRL, Before Memory Cleanup")
    gc.collect()
