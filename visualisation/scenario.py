...
def label_scenario(cfg_flat: dict) -> str:
    ...
    if slip_prob and slip_prob > 0:
        # If shaped reward under slip, make it explicit; else just 'noisy'
        return 'noisy_shaped' if reward_type == 'shaped' else 'noisy'
    if reward_type == 'shaped':
        return 'shaped'
    if num_trajectories is not None and num_trajectories <= 10:
        return 'fewshot'
    return 'baseline'