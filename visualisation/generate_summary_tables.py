...
def main():
    ...
    if 'env.slip_prob' not in df.columns:
        if 'slip_prob' in df.columns:
            df['env.slip_prob'] = df['slip_prob']
    if 'env.reward_type' not in df.columns:
        if 'reward_type' in df.columns:
            df['env.reward_type'] = df['reward_type']
    if 'expert.num_trajectories' not in df.columns:
        for k in ['num_trajectories', 'expert.n_trajectories', 'train.n_demos']:
            if k in df.columns:
                df['expert.num_trajectories'] = df[k]
                break
    # Optional join with scaling perf.csv (guarded) :contentReference[oaicite:1]{index=1}
    perf = _try_load_csv(os.path.join("results", "scaling", "perf.csv"))
    if perf is not None and {'method', 'size', 'seed', 'wall_clock_s'}.issubset(set(perf.columns)):
        # harmonize seed
        if 'train.seed' in df.columns and 'seed' not in df.columns:
            df['seed'] = df['train.seed']
        # extract numeric N from env.grid_size like "[7,7]" → 7
        if 'env.grid_size' in df.columns and 'size' not in df.columns:
            df['size'] = (
                df['env.grid_size'].astype(str)
                .str.extract(r'\[(\d+),\1\]')[0]
                .astype(float)
            )
        if 'size' in df.columns and 'seed' in df.columns and 'method' in df.columns:
            df = pd.merge(df, perf[['method', 'size', 'seed', 'wall_clock_s']],
                          how='left', on=['method', 'size', 'seed'])
