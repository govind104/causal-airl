...
j3=$(qsub -N confounded_gridworld -hold_jid $j2 scripts/run_confounded_gridworld.sh | awk '{print $3}')
j4=$(qsub -N cartpole_comparison  -hold_jid $j3 scripts/run_cartpole_comparison.sh  | awk '{print $3}')
j5=$(qsub -N generalisation_test  -hold_jid $j4 scripts/run_generalisation_test.sh  | awk '{print $3}')
j6=$(qsub -N generate_figures     -hold_jid $j5 scripts/run_generate_figures.sh     | awk '{print $3}')
